...
有了跨域限制：商场必须得到银行授权（CORS配置），才能查看你的卡
### 技术上的安全原因

```javascript
// 假设你登录了网上银行
// 你的浏览器里有银行的 cookie（包含登录信息）

// 然后你访问了一个恶意网站
// 这个网站用 JavaScript 发请求到银行：

fetch('https://bank.com/transfer', {
    method: 'POST',
    body: JSON.stringify({to: 'hacker', amount: 10000})
})
// 如果没有跨域限制，恶意网站就能利用你的 cookie 转账！
// 浏览器因为同源策略，阻止了这个请求
```
...
```javascript
// ✅ 同源请求 - 正常
fetch('http://localhost:5173/api/users')  // 同源，没问题

// ❌ 跨域请求 - 被阻止
fetch('http://localhost:8000/api/users')  // 端口不同，跨域！
```
...
```
浏览器: "我要去 http://api.example.com 拿数据"
浏览器: "先看看服务器让不让跨域..."
...
```
浏览器: "我要发 PUT 请求到 http://api.example.com"
浏览器: "先发个预检请求问问..."
...
**本质就是：** 浏览器为了安全阻止了跨域请求，你需要明确告诉浏览器"这个请求是安全的"，就是配置 CORS。
