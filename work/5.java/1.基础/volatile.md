...
还是这个例子，a = 1 ，flag = true 可能flag会在a之前执行，但是加了volatile， flag之前的操作只能在flag之前执行

volatile 的准确理解不是简单的“刷入主内存”，而是告诉编译器，这个变量可能被其他线程修改，不要随便缓存、消除或重排它的读写；建立线程之间的可见性和顺序保证；
一个线程写入后，其他线程读取这个变量时，必须遵守相应的内存同步规则。
