...
每次使用 Claude Code、Codex 或 Cherry Studio 前，必须先打开对应线路的“模型健康检测”页面。

确认当前需要使用的模型处于健康状态后，才能开始使用。
...
1. 不要继续反复提交任务；
2. 切换到其他健康线路；
3. 如果所有主线路均不可用，改用 OpenCode；
4. 如果需要生成图片，使用 WisArt。
...
> API Key 不应写在公共 SOP 中。使用者应向管理员单独获取，或者在受控的密码管理工具中查看。
...
你本次消息中给出的三个 API Key 都应视为已经泄露。请立即：

1. 登录对应服务商后台；
2. 删除或禁用原 Key；
3. 创建新 Key；
4. 更新 CC Switch 和 Cherry Studio；
5. 检查是否存在异常调用或额度消耗；
6. 后续截图只保留 Key 的前几位，其余全部打码。
